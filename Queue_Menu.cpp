...
Queue_Menu::Queue_Menu(Event* event, RenderWindow* window)
{
	...
	Toggle_Group_Queue.Toggle_Btn_Grp[0] = tog_Initialize;	Toggle_Group_Queue.Toggle_Btn_Grp[1] = tog_Add;	Toggle_Group_Queue.Toggle_Btn_Grp[2] = tog_Delete;
	Toggle_Group_Queue.Toggle_Btn_Grp[3] = tog_Search;

	Toggle_Group_Queue.n = 4;
	...
	{
		Toggle_Group_ser.tog_Grp_Head = new ToggleButton(tog_Search->pos_x + tog_Search->size_x + 20, tog_Search->pos_y, 150, 0, 2, "Position", Color::Black, Color::White, Color::Color(90, 90, 90, 155), Color::Color(90, 90, 90, 255), true, Color::Black);

		this->tog_ser_head = new ToggleButton(Toggle_Group_ser.tog_Grp_Head->pos_x, Toggle_Group_ser.tog_Grp_Head->pos_y + Toggle_Group_ser.tog_Grp_Head->size_y + 10, Toggle_Group_ser.tog_Grp_Head->size_x, 0, 0, "Front", Color::Black, Color::White, Color::Color(90, 90, 90, 155), Color::Color(90, 90, 90, 255), false, Color::Black);
		this->tog_ser_tail = new ToggleButton(Toggle_Group_ser.tog_Grp_Head->pos_x, Toggle_Group_ser.tog_Grp_Head->pos_y + Toggle_Group_ser.tog_Grp_Head->size_y * 2 + 10, Toggle_Group_ser.tog_Grp_Head->size_x, 0, 0, "Back", Color::Black, Color::White, Color::Color(90, 90, 90, 155), Color::Color(90, 90, 90, 255), false, Color::Black);
		
		Toggle_Group_ser.Toggle_Btn_Grp[0] = tog_ser_head;	Toggle_Group_ser.Toggle_Btn_Grp[1] = tog_ser_tail;
		Toggle_Group_ser.n = 2;

		Toggle_Group_ser.tog_Grp_Head->s2 = "";

		this->btn_ser_confirm = new Button(Toggle_Group_ser.tog_Grp_Head->pos_x + Toggle_Group_ser.tog_Grp_Head->size_x + 20, tog_Search->pos_y, 0, 0, "CONFIRM", Color::Black, Color::White, Color::Color(90, 90, 90, 155), Color::Color(90, 90, 90, 255), Color::Black);
	}
	...
	// Instruction
	{
		...
		Instruction[0] = "Make an Empty Queue";
		Instruction[1] = "Make an Queue with random size and values";
		Instruction[2] = "Make an Queue with inputed size and values";
		Instruction[3] = "Make an Queue with inputed values (for creating a Queue with size 1 please use Push function)";
		Instruction[4] = "Make an Queue with size and value imported from data file";

		Instruction[5] = "Add a Node to the Tail of the Queue";

		Instruction[6] = "Delete the Node at the Head of the Queue";

		Instruction[7] = "Get the value of the Head of the Queue and highlight it";

		Instruction[8] = "Get the value of the Tail of the Queue and highlight it";
	...
	anime = new Animation_Queue(event, &l, window);
...
void Queue_Menu::drawTrans(int start, int end, short CodeStatus)
{
...
void Queue_Menu::drawFrom(int Current, bool hasHead)
{
...
void Queue_Menu::Render()
{
	...
	{
		Toggle_Group_ser.render(window);
...
void Queue_Menu::update_add()
{
	...
	if (add_data_val != nothing && l.Size < 10 && box_add_val->btn_cofirm->isPressed())
	{
		...
		if (l.Size)
		{
			anime->Add_pos(11, add_data_val);
			drawFrom(0, 0);
		}

		l.addKth(New(add_data_val), 11);
...
{
	if (tog_ser_head->Toggled()) ser_data = 0, curInsId = 7;
	if (tog_ser_tail->Toggled()) ser_data = 11, curInsId = 8;

	if (btn_ser_confirm->isPressed() && ser_data != nothing)
	{
		tog_Search->disable();
...
void Queue_Menu::update(const Vector2f mousePos)
{
	...
	if (tog_Search->Toggled())
	{
		Toggle_Group_ser.filter(mousePos, event);

		this->btn_ser_confirm->update(mousePosWindowf, event);

		update_search();
	}
	else Toggle_Group_ser.clearAll();

	if (Toggle_Group_Queue.checkEmpty()) curInsId = 9;

	loadInstruction();
}

void Queue_Menu::loadInstruction()
{
